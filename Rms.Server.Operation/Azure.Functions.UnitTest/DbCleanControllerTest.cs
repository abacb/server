...
using Rms.Server.Operation.Azure.Functions.DbCleaner;
...
namespace Azure.Functions.DbCleanerTest
{
    ...
    public class DbCleanControllerTest
    {
        ...
        /// <remarks>削除されないことを確認するテーブル群。削除対象テーブルとリレーションがあるテーブルを選出する。依存関係順（依存する方が先）。</remarks>
        private static readonly string[] NonTargetTableNames = new string[]
        {
            "operation.MT_NETWORK_ROUTE",
            "operation.DT_INSTALL_BASE",
            "operation.DT_EQUIPMENT",
        };
