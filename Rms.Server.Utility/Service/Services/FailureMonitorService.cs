...
namespace Rms.Server.Utility.Service.Services
{
    ...
    /// </summary>
    public class FailureMonitorService : IFailureMonitorService
    {
        /// <summary>
        /// 設定
        /// </summary>
        ...
        /// 故障発生監視アラーム定義リポジトリ
        /// </summary>
        private readonly IDtAlarmDefFailureMonitorRepository _dtAlarmDefFailureMonitorRepository;
        ...
        /// <param name="dtAlarmDefFailureMonitorRepository">故障発生監視アラーム定義リポジトリ</param>
        /// <param name="queueRepository">Queueリポジトリ</param>
        /// <param name="failureRepository">Failureストレージリポジトリ</param>
        public FailureMonitorService(
            UtilityAppSettings settings,
            ILogger<FailureMonitorService> logger,
            ITimeProvider timeProvider,
            IDtAlarmDefFailureMonitorRepository dtAlarmDefFailureMonitorRepository,
            ...
            IFailureRepository failureRepository)
        {
            ...
            Assert.IfNull(dtAlarmDefFailureMonitorRepository);
            ...
            _dtAlarmDefFailureMonitorRepository = dtAlarmDefFailureMonitorRepository;
        ...
        public bool ReadAlarmDefinition(ErrorLog errorLog, string messageId, out IEnumerable<DtAlarmDefFailureMonitor> models)
        {
            models = null;

            try
            {
                _logger.EnterJson("{0}", new { errorLog, messageId });

                // Sq1.1.1: 故障発生監視アラーム定義を取得
                models = _dtAlarmDefFailureMonitorRepository.ReadDtAlarmDefFailureMonitor(errorLog);
            ...
            catch (RmsException e)
            {
                ...
                _logger.Error(e, nameof(Resources.UT_FLM_FLM_003), new object[] { messageId });
                return false;
            }
            finally
            {
        ...
        /// <returns>成功した場合true、失敗した場合falseを返す</returns>
        public bool CreateAndEnqueueAlarmInfo(ErrorLog errorLog, string messageId, IEnumerable<DtAlarmDefFailureMonitor> alarmDef)
        {
            bool result = true;
            _logger.EnterJson("{0}", new { errorLog, messageId, alarmDef });
            ...
            foreach (var alarm in alarmDef)
            {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(alarmDescription))
                    {
                        ...
                        {
                            alarmDescription = string.Format(alarmDescription, errorLog.ErrorContents);
                        }
                    }
                    ...
                    {
                        SourceEquipmentUid = errorLog.SourceEquipmentUid,
                        TypeCode = errorLog.TypeCode,
                        ...
                        EventDatetime = errorLog.EventDt,
                ...
                catch (Exception e)
                {
                    ...
                    _logger.Error(e, string.IsNullOrEmpty(message) ? nameof(Resources.UT_FLM_FLM_005) : nameof(Resources.UT_FLM_FLM_006), new object[] { messageId });
                    result = false;
                }
            }
        ...
        public void UpdateToFailureStorage(string messageSchemaId, string messageId, string message)
        {
            ...
            catch (RmsException e)
            {
                ...
                _logger.Error(e, nameof(Resources.UT_FLM_FLM_008), new object[] { messageId, message });
            }
            finally
            {
                _logger.Leave();
