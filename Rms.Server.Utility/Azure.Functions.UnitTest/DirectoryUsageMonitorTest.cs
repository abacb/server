...
using Rms.Server.Utility.Azure.Functions.DirectoryUsageMonitor;
...
{
    /// <summary>
    /// DirectoryUsageMonitorTest
    /// </summary>
    [TestClass]
    public class DirectoryUsageMonitorTest
    {
        /// <summary>テスト結果格納先のルートフォルダ</summary>
        private static readonly string TestResultRootDir = string.Format(@"TestResult.{0:yyyyMMddHHmmss}\{1}", DateTime.Now, typeof(DirectoryUsageMonitorController).Name);
        ...
        private static readonly Dictionary<string, string> DefaultAppSettingValues = new Dictionary<string, string>()
        {
            ...
            { "SystemName", "rms" },
            { "SubSystemName", "directoryusagemonitor" },
        ...
        private DirectoryUsageMonitorController target;
        ...
        public void TestInitialize()
        {
            ...
            using (SqlConnection connection = new SqlConnection(settings.PrimaryDbConnectionString))
            {
                ...
                {
                    connection.Open();
                    using (SqlCommand command = new SqlCommand("TRUNCATE TABLE utility.DT_ALARM_DEF_DIRECTORY_USAGE_MONITOR;", connection))
                    {
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\Functions_DirectoryUsageMonitor_ReceiveDirectoryUsage.csv")]
