...
{
    /// <summary>
    /// DeviceConnectionMonitorControllerTest
    /// </summary>
    [TestClass]
    public class DeviceConnectionMonitorControllerTest
    {
        ...
        private DeviceConnectionMonitorController target;
        ...
        public string ClearTableCommand
        {
            get
            {
                ...
                sb.AppendLine("DELETE FROM core.MT_CONNECT_STATUS;");
                sb.AppendLine("DBCC CHECKIDENT ('core.MT_CONNECT_STATUS', RESEED, 0);");
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\Functions_DeviceConnectionMonitor_MonitorDeviceConnectionTable.csv")]
        public void MonitorDeviceConnectionTableTest(string no, string in_InitializeSqlSet, string expected_AlarmQueueSet, string expected_LogMessages, string remarks)
        {
            ...
            // テスト実行
            target.MonitorDeviceConnectionTable(null, new TestLogger<DeviceConnectionMonitorController>(actual_logs));
        ...
        private void DependencyInjection(Dictionary<string, string> appSettings = null, List<TestLog> testLogs = null)
        {
            ...
            builder.ServiceCollection.AddTransient<DeviceConnectionMonitorController>();
            ...
            if (testLogs != null)
            {
                builder.ServiceCollection.AddSingleton<ILogger<DeviceConnectionMonitorService>>(new TestLogger<DeviceConnectionMonitorService>(testLogs));
            }

            ServiceProvider provider = builder.Build();
            settings = provider.GetService<UtilityAppSettings>();
            target = provider.GetService<DeviceConnectionMonitorController>();
        }
    }
}
