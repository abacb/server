...
using Rms.Server.Utility.Azure.Functions.FailureMonitor;
...
{
    /// <summary>
    /// FailureMonitorControllerTest
    /// </summary>
    [TestClass]
    public class FailureMonitorControllerTest
    {
        /// <summary>テスト結果格納先のルートフォルダ</summary>
        private static readonly string TestResultRootDir = string.Format(@"TestResult.{0:yyyyMMddHHmmss}\{1}", DateTime.Now, typeof(FailureMonitorController).Name);
        ...
        private FailureMonitorController target;
        ...
        public void TestInitialize()
        {
            ...
            using (SqlConnection connection = new SqlConnection(settings.PrimaryDbConnectionString))
            {
                ...
                {
                    connection.Open();
                    using (SqlCommand command = new SqlCommand("TRUNCATE TABLE utility.DT_ALARM_DEF_FAILURE_MONITOR;", connection))
                    {
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\Functions_FailureMonitor_ReceiveErrorLog.csv")]
        public void ReceiveErrorLogTest(string no, string in_InitializeSqlSet, string in_Message, string expected_AlarmQueueSet, string expected_FailureBlobFileSet, string expected_LogMessages, string remarks)
        {
            ...
            // テスト実行
            target.ReceiveErrorLog(in_Message, new TestLogger<FailureMonitorController>(actual_logs));
        ...
        private void DependencyInjection(Dictionary<string, string> appSettings = null, List<TestLog> testLogs = null)
        {
            ...
            builder.ServiceCollection.AddTransient<FailureMonitorController>();
            ...
            if (testLogs != null)
            {
                builder.ServiceCollection.AddSingleton<ILogger<FailureMonitorService>>(new TestLogger<FailureMonitorService>(testLogs));
            }
            ...
            target = provider.GetService<FailureMonitorController>();
        }
    }
}
