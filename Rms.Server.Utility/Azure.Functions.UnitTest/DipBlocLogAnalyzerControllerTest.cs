...
using Rms.Server.Utility.Azure.Functions.DipBlocLogAnalyzer;
...
{
    /// <summary>
    /// DipBlocLogAnalyzerControllerTest
    /// </summary>
    [TestClass]
    public class DipBlocLogAnalyzerControllerTest
    {
        /// <summary>テスト結果格納先のルートフォルダ</summary>
        private static readonly string TestResultRootDir = string.Format(@"TestResult.{0:yyyyMMddHHmmss}\{1}", DateTime.Now, typeof(DipBlocLogAnalyzerController).Name);
        ...
        private static readonly Dictionary<string, string> DefaultAppSettingValues = new Dictionary<string, string>()
        {
            ...
            { "SystemName", "rms" },
            { "SubSystemName", "dipblocloganalyzer" },
        ...
        private DipBlocLogAnalyzerController target;
        ...
        public void TestInitialize()
        {
            ...
            using (SqlConnection connection = new SqlConnection(settings.PrimaryDbConnectionString))
            {
                ...
                {
                    connection.Open();
                    using (SqlCommand command = new SqlCommand("TRUNCATE TABLE utility.DT_BLOCLOG_ANALYSIS_RESULT;", connection))
                    {
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\Functions_DipBlocLogAnalyzer_ReceiveDipBlocLog.csv")]
        public void ReceiveDipBlocLogTest(string no, string in_InitializeSqlSet, string in_Message, string expected_TableData, string expected_FailureBlobFileSet, string expected_LogMessages, string remarks)
        {
            ...
            // テスト実行
            target.ReceiveDipBlocLog(in_Message, new TestLogger<DipBlocLogAnalyzerController>(actual_logs));
        ...
        private void DependencyInjection(Dictionary<string, string> appSettings = null, List<TestLog> testLogs = null)
        {
            ...
            builder.ServiceCollection.AddTransient<DipBlocLogAnalyzerController>();
            ...
            if (testLogs != null)
            {
                builder.ServiceCollection.AddSingleton<ILogger<DipBlocLogAnalyzerService>>(new TestLogger<DipBlocLogAnalyzerService>(testLogs));
            }
            ...
            target = provider.GetService<DipBlocLogAnalyzerController>();
        }
    }
}
