...
{
    /// <summary>
    /// TubeCurrentDeteriorationPremonitorControllerTest
    /// </summary>
    [TestClass]
    public class TubeCurrentDeteriorationPremonitorControllerTest
    {
        /// <summary>テスト結果格納先のルートフォルダ</summary>
        private static readonly string TestResultRootDir = string.Format(@"TestResult.{0:yyyyMMddHHmmss}\{1}", DateTime.Now, typeof(TubeCurrentDeteriorationPremonitorController).Name);
        ...
        private static readonly Dictionary<string, string> DefaultAppSettingValues = new Dictionary<string, string>()
        {
            ...
            { "SystemName", "rms" },
            { "SubSystemName", "tubecurrentdeteriorationpremonitor" },
        ...
        private TubeCurrentDeteriorationPremonitorController target;
        ...
        public void TestInitialize()
        {
            ...
            using (SqlConnection connection = new SqlConnection(settings.PrimaryDbConnectionString))
            {
                ...
                {
                    connection.Open();
                    using (SqlCommand command = new SqlCommand("TRUNCATE TABLE utility.DT_ALARM_DEF_TUBE_CURRENT_DETERIORATION_PREMONITOR;", connection))
                    {
        ...
        /// <summary>
        /// ReceiveTubeCurrentDeteriorationPredictiveResutLogTest
        /// </summary>
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\Functions_TubeCurrentDeteriorationPremonitor_ReceiveTubeCurrentDeteriorationPredictiveResutLog.csv")]
        public void ReceiveTubeCurrentDeteriorationPredictiveResutLogTest(string no, string in_InitializeSqlSet, string in_Message, string expected_AlarmQueueSet, string expected_FailureBlobFileSet, string expected_LogMessages, string remarks)
        {
            ...
            // テスト実行
            target.ReceiveTubeCurrentDeteriorationPredictiveResutLog(in_Message, new TestLogger<TubeCurrentDeteriorationPremonitorController>(actual_logs));
        ...
        private void DependencyInjection(Dictionary<string, string> appSettings = null, List<TestLog> testLogs = null)
        {
            ...
            builder.ServiceCollection.AddTransient<TubeCurrentDeteriorationPremonitorController>();
            ...
            if (testLogs != null)
            {
                builder.ServiceCollection.AddSingleton<ILogger<TubeCurrentDeteriorationPremonitorService>>(new TestLogger<TubeCurrentDeteriorationPremonitorService>(testLogs));
            }
            ...
            target = provider.GetService<TubeCurrentDeteriorationPremonitorController>();
        }
    }
}
