...
{
    /// <summary>
    /// TubeDeteriorationPremonitorControllerTest
    /// </summary>
    [TestClass]
    public class TubeDeteriorationPremonitorControllerTest
    {
        /// <summary>テスト結果格納先のルートフォルダ</summary>
        private static readonly string TestResultRootDir = string.Format(@"TestResult.{0:yyyyMMddHHmmss}\{1}", DateTime.Now, typeof(TubeDeteriorationPremonitorController).Name);
        ...
        private static readonly Dictionary<string, string> DefaultAppSettingValues = new Dictionary<string, string>()
        {
            ...
            { "SystemName", "rms" },
            { "SubSystemName", "tubedeteriorationpremonitor" },
        ...
        private TubeDeteriorationPremonitorController target;
        ...
        public void TestInitialize()
        {
            ...
            using (SqlConnection connection = new SqlConnection(settings.PrimaryDbConnectionString))
            {
                ...
                {
                    connection.Open();
                    using (SqlCommand command = new SqlCommand("TRUNCATE TABLE utility.DT_ALARM_DEF_TUBE_DETERIORATION_PREMONITOR;", connection))
                    {
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\Functions_TubeDeteriorationPremonitor_ReceiveTubeDeteriorationPredictiveResutLog.csv")]
        public void ReceiveTubeDeteriorationPredictiveResutLogTest(string no, string in_InitializeSqlSet, string in_Message, string expected_AlarmQueueSet, string expected_FailureBlobFileSet, string expected_LogMessages, string remarks)
        {
            ...
            // テスト実行
            target.ReceiveTubeDeteriorationPredictiveResutLog(in_Message, new TestLogger<TubeDeteriorationPremonitorController>(actual_logs));
        ...
        private void DependencyInjection(Dictionary<string, string> appSettings = null, List<TestLog> testLogs = null)
        {
            ...
            builder.ServiceCollection.AddTransient<TubeDeteriorationPremonitorController>();
            ...
            if (testLogs != null)
            {
                builder.ServiceCollection.AddSingleton<ILogger<TubeDeteriorationPremonitorService>>(new TestLogger<TubeDeteriorationPremonitorService>(testLogs));
            }
            ...
            target = provider.GetService<TubeDeteriorationPremonitorController>();
        }
    }
}
