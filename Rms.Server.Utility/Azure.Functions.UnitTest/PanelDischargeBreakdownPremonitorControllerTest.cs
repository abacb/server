...
using Rms.Server.Utility.Azure.Functions.PanelDischargeBreakdownPremonitor;
...
{
    /// <summary>
    /// PanelDischargeBreakdownPremonitorControllerTest
    /// </summary>
    [TestClass]
    public class PanelDischargeBreakdownPremonitorControllerTest
    {
        /// <summary>テスト結果格納先のルートフォルダ</summary>
        private static readonly string TestResultRootDir = string.Format(@"TestResult.{0:yyyyMMddHHmmss}\{1}", DateTime.Now, typeof(PanelDischargeBreakdownPremonitorController).Name);
        ...
        private static readonly Dictionary<string, string> DefaultAppSettingValues = new Dictionary<string, string>()
        {
            ...
            { "SystemName", "rms" },
            { "SubSystemName", "paneldischargebreakdownpremonitor" },
        ...
        private PanelDischargeBreakdownPremonitorController target;
        ...
        public void TestInitialize()
        {
            ...
            using (SqlConnection connection = new SqlConnection(settings.PrimaryDbConnectionString))
            {
                ...
                {
                    connection.Open();
                    using (SqlCommand command = new SqlCommand("TRUNCATE TABLE utility.DT_ALARM_DEF_PANEL_DISCHARGE_BREAKDOWN_PREMONITOR;", connection))
                    {
        ...
        /// <summary>
        /// ReceivePanelDischargeBreakdownPredictiveResutLogTest
        /// </summary>
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\Functions_PanelDischargeBreakdownPremonitor_ReceivePanelDischargeBreakdownPredictiveResutLog.csv")]
        public void ReceivePanelDischargeBreakdownPredictiveResutLogTest(string no, string in_InitializeSqlSet, string in_Message, string expected_AlarmQueueSet, string expected_FailureBlobFileSet, string expected_LogMessages, string remarks)
        {
            ...
            // テスト実行
            target.ReceivePanelDischargeBreakdownPredictiveResutLog(in_Message, new TestLogger<PanelDischargeBreakdownPremonitorController>(actual_logs));
        ...
        private void DependencyInjection(Dictionary<string, string> appSettings = null, List<TestLog> testLogs = null)
        {
            ...
            builder.ServiceCollection.AddTransient<PanelDischargeBreakdownPremonitorController>();
            ...
            if (testLogs != null)
            {
                builder.ServiceCollection.AddSingleton<ILogger<PanelDischargeBreakdownPremonitorService>>(new TestLogger<PanelDischargeBreakdownPremonitorService>(testLogs));
            }
            ...
            target = provider.GetService<PanelDischargeBreakdownPremonitorController>();
        }
    }
}
