...
using Rms.Server.Utility.Azure.Functions.TemperatureSensorMonitor;
...
{
    /// <summary>
    /// TemperatureSensorMonitorControllerTest
    /// </summary>
    [TestClass]
    public class TemperatureSensorMonitorControllerTest
    {
        /// <summary>テスト結果格納先のルートフォルダ</summary>
        private static readonly string TestResultRootDir = string.Format(@"TestResult.{0:yyyyMMddHHmmss}\{1}", DateTime.Now, typeof(TemperatureSensorMonitorController).Name);
        ...
        private static readonly Dictionary<string, string> DefaultAppSettingValues = new Dictionary<string, string>()
        {
            ...
            { "SystemName", "rms" },
            { "SubSystemName", "temperaturesensormonitor" },
        ...
        private TemperatureSensorMonitorController target;
        ...
        public void TestInitialize()
        {
            ...
            using (SqlConnection connection = new SqlConnection(settings.PrimaryDbConnectionString))
            {
                ...
                {
                    connection.Open();
                    using (SqlCommand command = new SqlCommand("TRUNCATE TABLE utility.DT_ALARM_DEF_TEMPERATURE_SENSOR_LOG_MONITOR;", connection))
                    {
        ...
        /// <summary>
        /// ReceiveTemperatureSensorLogTest
        /// </summary>
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\Functions_TemperatureSensorMonitor_ReceiveTemperatureSensorLog.csv")]
        public void ReceiveTemperatureSensorLogTest(string no, string in_InitializeSqlSet, string in_Message, string expected_AlarmQueueSet, string expected_FailureBlobFileSet, string expected_LogMessages, string remarks)
        {
            ...
            // テスト実行
            target.ReceiveTemperatureSensorLog(in_Message, new TestLogger<TemperatureSensorMonitorController>(actual_logs));
        ...
        private void DependencyInjection(Dictionary<string, string> appSettings = null, List<TestLog> testLogs = null)
        {
            ...
            builder.ServiceCollection.AddTransient<TemperatureSensorMonitorController>();
            ...
            if (testLogs != null)
            {
                builder.ServiceCollection.AddSingleton<ILogger<TemperatureSensorMonitorService>>(new TestLogger<TemperatureSensorMonitorService>(testLogs));
            }
            ...
            target = provider.GetService<TemperatureSensorMonitorController>();
        }
    }
}
