...
{
    /// <summary>
    /// CalibrationPremonitorControllerTest
    /// </summary>
    [TestClass]
    public class CalibrationPremonitorControllerTest
    {
        /// <summary>テスト結果格納先のルートフォルダ</summary>
        private static readonly string TestResultRootDir = string.Format(@"TestResult.{0:yyyyMMddHHmmss}\{1}", DateTime.Now, typeof(CalibrationPremonitorController).Name);
        ...
        private static readonly Dictionary<string, string> DefaultAppSettingValues = new Dictionary<string, string>()
        {
            ...
            { "SystemName", "rms" },
            { "SubSystemName", "calibrationpremonitor" },
        ...
        private CalibrationPremonitorController target;
        ...
        public void TestInitialize()
        {
            ...
            using (SqlConnection connection = new SqlConnection(settings.PrimaryDbConnectionString))
            {
                ...
                {
                    connection.Open();
                    using (SqlCommand command = new SqlCommand("TRUNCATE TABLE utility.DT_ALARM_DEF_CALIBRATION_PREMONITOR;", connection))
                    {
        ...
        /// <summary>
        /// ReceiveCalibrationPredictiveResutLogTest
        /// </summary>
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\Functions_CalibrationPremonitor_ReceiveCalibrationPredictiveResutLog.csv")]
        public void ReceiveCalibrationPredictiveResutLogTest(string no, string in_InitializeSqlSet, string in_Message, string expected_AlarmQueueSet, string expected_FailureBlobFileSet, string expected_LogMessages, string remarks)
        {
            ...
            // テスト実行
            target.ReceiveCalibrationPredictiveResutLog(in_Message, new TestLogger<CalibrationPremonitorController>(actual_logs));
        ...
        private void DependencyInjection(Dictionary<string, string> appSettings = null, List<TestLog> testLogs = null)
        {
            ...
            builder.ServiceCollection.AddTransient<CalibrationPremonitorController>();
            ...
            if (testLogs != null)
            {
                builder.ServiceCollection.AddSingleton<ILogger<CalibrationPremonitorService>>(new TestLogger<CalibrationPremonitorService>(testLogs));
            }
            ...
            target = provider.GetService<CalibrationPremonitorController>();
        }
    }
}
