...
{
    /// <summary>
    /// InstallMonitorControllerTest
    /// </summary>
    [TestClass]
    public class InstallMonitorControllerTest
    {
        /// <summary>テスト結果格納先のルートフォルダ</summary>
        private static readonly string TestResultRootDir = string.Format(@"TestResult.{0:yyyyMMddHHmmss}\{1}", DateTime.Now, typeof(InstallMonitorController).Name);
        ...
        private InstallMonitorController target;
        ...
        public void TestInitialize()
        {
            ...
            using (SqlConnection connection = new SqlConnection(settings.PrimaryDbConnectionString))
            {
                ...
                {
                    connection.Open();
                    using (SqlCommand command = new SqlCommand("TRUNCATE TABLE utility.DT_ALARM_DEF_INSTALL_RESULT_MONITOR;", connection))
                    {
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\Functions_InstallMonitor_ReceiveInstallResult.csv")]
        public void ReceiveInstallResultTest(string no, string in_InitializeSqlSet, string in_Message, string expected_AlarmQueueSet, string expected_FailureBlobFileSet, string expected_LogMessages, string remarks)
        {
            ...
            // テスト実行
            target.ReceiveInstallResult(in_Message, new TestLogger<InstallMonitorController>(actual_logs));
        ...
        private void DependencyInjection(Dictionary<string, string> appSettings = null, List<TestLog> testLogs = null)
        {
            ...
            builder.ServiceCollection.AddTransient<InstallMonitorController>();
            ...
            if (testLogs != null)
            {
                builder.ServiceCollection.AddSingleton<ILogger<InstallMonitorService>>(new TestLogger<InstallMonitorService>(testLogs));
            }
            ...
            target = provider.GetService<InstallMonitorController>();
        }
    }
}
