...
using Rms.Server.Utility.Azure.Functions.PanelDefectPremonitor;
...
{
    /// <summary>
    /// PanelDefectPremonitorControllerTest
    /// </summary>
    [TestClass]
    public class PanelDefectPremonitorControllerTest
    {
        /// <summary>テスト結果格納先のルートフォルダ</summary>
        private static readonly string TestResultRootDir = string.Format(@"TestResult.{0:yyyyMMddHHmmss}\{1}", DateTime.Now, typeof(PanelDefectPremonitorController).Name);
        ...
        private static readonly Dictionary<string, string> DefaultAppSettingValues = new Dictionary<string, string>()
        {
            ...
            { "SystemName", "rms" },
            { "SubSystemName", "paneldefectpremonitor" },
        ...
        private PanelDefectPremonitorController target;
        ...
        public void TestInitialize()
        {
            ...
            using (SqlConnection connection = new SqlConnection(settings.PrimaryDbConnectionString))
            {
                ...
                {
                    connection.Open();
                    using (SqlCommand command = new SqlCommand("TRUNCATE TABLE utility.DT_ALARM_DEF_PANEL_DEFECT_PREMONITOR;", connection))
                    {
        ...
        /// <summary>
        /// ReceivePanelDefectPredictiveResutLogTest
        /// </summary>
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\Functions_PanelDefectPremonitor_ReceivePanelDefectPredictiveResutLog.csv")]
        public void ReceivePanelDefectPredictiveResutLogTest(string no, string in_InitializeSqlSet, string in_Message, string expected_AlarmQueueSet, string expected_FailureBlobFileSet, string expected_LogMessages, string remarks)
        {
            ...
            // テスト実行
            target.ReceivePanelDefectPredictiveResutLog(in_Message, new TestLogger<PanelDefectPremonitorController>(actual_logs));
        ...
        private void DependencyInjection(Dictionary<string, string> appSettings = null, List<TestLog> testLogs = null)
        {
            ...
            builder.ServiceCollection.AddTransient<PanelDefectPremonitorController>();
            ...
            if (testLogs != null)
            {
                builder.ServiceCollection.AddSingleton<ILogger<PanelDefectPremonitorService>>(new TestLogger<PanelDefectPremonitorService>(testLogs));
            }
            ...
            target = provider.GetService<PanelDefectPremonitorController>();
        }
    }
}
