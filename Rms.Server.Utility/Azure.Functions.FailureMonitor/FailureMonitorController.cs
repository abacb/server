...
namespace Rms.Server.Utility.Azure.Functions.FailureMonitor
{
    /// <summary>
    /// 故障発生監視アプリ
    /// </summary>
    public class FailureMonitorController
    {
        ...
        /// イベントハブ名（メッセージスキーマID）
        /// </summary>
        private const string EventHubName = "ms-010";
        ...
        /// 故障発生監視サービスクラス
        /// </summary>
        private readonly IFailureMonitorService _service;

        /// <summary>
        /// コンストラクタ
        /// </summary>
        /// <param name="settings">設定</param>
        /// <param name="service">故障発生監視サービスクラス</param>
        public FailureMonitorController(UtilityAppSettings settings, IFailureMonitorService service)
        {
        ...
        /// エラーログを受信する
        /// </summary>
        /// <remarks>sd 07-01.故障発生監視</remarks>
        /// <param name="message">メッセージ</param>
        /// <param name="log">ロガー</param>
        [FunctionName("FailureMonitor")]
        public void ReceiveErrorLog([EventHubTrigger(EventHubName, Connection = "ConnectionString")] string message, ILogger log)
        {
            ...
            try
            {
                ...
                foreach (var eventSchema in eventSchemaList)
                {
                    ...
                    try
                    {
                        ...
                        var errorLog = JsonConvert.DeserializeObject<ErrorLog>(deviceTelemetry.Body.ToStringJson());

                        // バリデーション
                        Validator.ValidateObject(errorLog, new ValidationContext(errorLog, null, null));

                        // Sq1.1.1: 故障発生監視アラーム定義を取得
                        if (_service.ReadAlarmDefinition(errorLog, messageId, out IEnumerable<DtAlarmDefFailureMonitor> models))
                        {
                            ...
                            {
                                // アラーム生成不要
                                log.Info(nameof(Resources.UT_FLM_FLM_004), new object[] { messageId });
                                continue;
                            }
                            else
                            {
                                ...
                                // Sq1.1.4: キューを登録する
                                if (_service.CreateAndEnqueueAlarmInfo(errorLog, messageId, models))
                                {
                                    // アラームキュー登録完了
                                    log.Info(nameof(Resources.UT_FLM_FLM_007), new object[] { messageId });
                                    continue;
                                }
                            }
                        }
                    ...
                    catch (ValidationException e)
                    {
                        ...
                        log.Error(e, nameof(Resources.UT_FLM_FLM_002), new object[] { messageId, e.Message });
                    }
                    catch (Exception e)
                    {
                        log.Error(e, nameof(Resources.UT_FLM_FLM_001), new object[] { messageId });
                    }
            ...
            {
                log.Error(e, nameof(Resources.UT_FLM_FLM_009), new object[] { e.Message });
            ...
            {
                log.Error(e, nameof(Resources.UT_FLM_FLM_001), new object[] { null });
