...
namespace Rms.Server.Utility.Azure.Functions.DirectoryUsageMonitor
{
    /// <summary>
    /// ディレクトリ使用量監視アプリ
    /// </summary>
    public class DirectoryUsageMonitorController
    {
        /// <summary>
        /// イベントハブ名（メッセージスキーマID）
        /// </summary>
        private const string EventHubName = "ms-026";
        ...
        /// ディレクトリ使用量監視サービスクラス
        /// </summary>
        private readonly IDirectoryUsageMonitorService _service;
        ...
        /// ディレクトリ使用量を受信する
        /// </summary>
        /// <remarks>sd 07-12.ディレクトリ使用量監視</remarks>
        /// <param name="message">メッセージ</param>
        /// <param name="log">ロガー</param>
        [FunctionName("DirectoryUsageMonitor")]
        public void ReceiveDirectoryUsage([EventHubTrigger(EventHubName, Connection = "ConnectionString")] string message, ILogger log)
        {
            ...
            try
            {
                ...
                foreach (var eventSchema in eventSchemaList)
                {
                    ...
                    try
                    {
                        ...
                        var directoryUsage = JsonConvert.DeserializeObject<DirectoryUsage>(deviceTelemetry.Body.ToStringJson());

                        // バリデーション
                        Validator.ValidateObject(directoryUsage, new ValidationContext(directoryUsage, null, null));

                        // Sq1.1.1: ディレクトリ通信監視アラーム定義を取得
                        if (_service.ReadAlarmDefinition(directoryUsage, messageId, out IEnumerable<DtAlarmDefDirectoryUsageMonitor> models))
                        {
                            ...
                            {
                                // アラーム生成不要
                                log.Info(nameof(Resources.UT_DUM_DUM_004), new object[] { messageId });
                                continue;
                            }
                            else
                            {
                                ...
                                // Sq1.1.4: キューを登録する
                                if (_service.CreateAndEnqueueAlarmInfo(directoryUsage, messageId, models))
                                {
                                    // アラームキュー登録完了
                                    log.Info(nameof(Resources.UT_DUM_DUM_007), new object[] { messageId });
                                    continue;
                                }
                            }
                        }
                    ...
                    catch (ValidationException e)
                    {
                        ...
                        log.Error(e, nameof(Resources.UT_DUM_DUM_002), new object[] { messageId, e.Message });
                    }
                    catch (Exception e)
                    {
                        log.Error(e, nameof(Resources.UT_DUM_DUM_001), new object[] { messageId });
                    }
            ...
            {
                log.Error(e, nameof(Resources.UT_DUM_DUM_009), new object[] { e.Message });
            ...
            {
                log.Error(e, nameof(Resources.UT_DUM_DUM_001), new object[] { null });
