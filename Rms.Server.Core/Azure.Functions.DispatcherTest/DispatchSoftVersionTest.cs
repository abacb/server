...
{
    /// <summary>
    /// DispatchSoftVersionTest
    /// </summary>
    [TestClass]
    public class DispatchSoftVersionTest
    {
        /// <summary>テスト結果格納先のルートフォルダ</summary>
        private static readonly string TestResultRootDir = string.Format(@"TestResult.{0:yyyyMMddHHmmss}\{1}", DateTime.Now, nameof(DispatchController.DispatchSoftVersion));
        ...
        private static readonly string FailureBlobReplacePath = @"\ms-025\";
        ...
        [DataTestMethod]
        [CsvDataSourece(@"TestCases\DispatchController_DispatchSoftVersion.csv")]
        public void SoftVersionTest(string no, string in_InitializeSqlSet, string in_Message, string expected_TableData, string expected_FailureBlobFileSet, string expected_LogMessages, string expected_WithoutError, string in_AppSettings, string error_Method, string remarks)
        {
            ...
            // テスト実行
            _target.DispatchSoftVersion(in_Message, new TestLogger<DispatchController>(actual_logs));
