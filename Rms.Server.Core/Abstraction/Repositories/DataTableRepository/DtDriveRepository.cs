...
namespace Rms.Server.Core.Abstraction.Repositories
{
    ...
    /// DT_DRIVEテーブルのリポジトリ
    /// </summary>
    public partial class DtDriveRepository : IDtDriveRepository
    {
        /// <summary>ロガー</summary>
        private readonly ILogger<DtDriveRepository> _logger;
        ...
        public DtDriveRepository(ILogger<DtDriveRepository> logger, ITimeProvider timePrivder, DBPolly dbPolly, AppSettings appSettings)
        {
        ...
        public DtDrive CreateDtDriveIfAlreadyMessageThrowEx(DtDrive inData)
        {
            DtDrive model = null;
            try
            {
                ...
                DBAccessor.Models.DtDrive entity = new DBAccessor.Models.DtDrive(inData);
                ...
                _dbPolly.Execute(() =>
                {
                    ...
                    {
                        var addedAlready = db.DtDrive.FirstOrDefault(x => x.MessageId == inData.MessageId);
                        if (addedAlready != null)
                        {
                            throw new RmsAlreadyExistException(string.Format("MessageId [{0}] はすでに追加済みです。", inData.MessageId));
                        }

                        var dbdata = db.DtDrive.Add(entity).Entity;
            ...
            {
                throw new RmsException("DT_DRIVEテーブルへのInsertに失敗しました。", e);
            }
            finally
            {
