...
{
    /// <summary>
    /// DT_DISK_DRIVEテーブルのリポジトリ
    /// </summary>
    public partial class DtDiskDriveRepository : IDtDiskDriveRepository
    {
        /// <summary>ロガー</summary>
        private readonly ILogger<DtDiskDriveRepository> _logger;
        ...
        public DtDiskDriveRepository(ILogger<DtDiskDriveRepository> logger, ITimeProvider timePrivder, DBPolly dbPolly, AppSettings appSettings)
        {
        ...
        public DtDiskDrive CreateDtDiskDriveIfAlreadyMessageThrowEx(DtDiskDrive inData)
        {
            DtDiskDrive model = null;
            try
            {
                _logger.EnterJson("{0}", inData);
    
                DBAccessor.Models.DtDiskDrive entity = new DBAccessor.Models.DtDiskDrive(inData);
                ...
                _dbPolly.Execute(() =>
                {
                    ...
                    {
                        var addedAlready = db.DtDiskDrive.FirstOrDefault(x => x.MessageId == inData.MessageId);
                        if (addedAlready != null)
                        {
                            throw new RmsAlreadyExistException(string.Format("MessageId [{0}] はすでに追加済みです。", inData.MessageId));
                        }

                        var dbdata = db.DtDiskDrive.Add(entity).Entity;
            ...
            {
                throw new RmsException("DT_DISK_DRIVEテーブルへのInsertに失敗しました。", e);
            }
            finally
            {
