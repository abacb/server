...
{
    /// <summary>
    /// DT_DXA_QC_LOGテーブルのリポジトリ
    /// </summary>
    public partial class DtDxaQcLogRepository : IDtDxaQcLogRepository
    {
        /// <summary>ロガー</summary>
        private readonly ILogger<DtDxaQcLogRepository> _logger;
        ...
        public DtDxaQcLogRepository(ILogger<DtDxaQcLogRepository> logger, ITimeProvider timePrivder, DBPolly dbPolly, AppSettings appSettings)
        {
        ...
        public DtDxaQcLog CreateDtDxaQcLogIfAlreadyMessageThrowEx(DtDxaQcLog inData)
        {
            DtDxaQcLog model = null;
            try
            {
                _logger.EnterJson("{0}", inData);
    
                DBAccessor.Models.DtDxaQcLog entity = new DBAccessor.Models.DtDxaQcLog(inData);
                ...
                _dbPolly.Execute(() =>
                {
                    ...
                    {
                        var addedAlready = db.DtDxaQcLog.FirstOrDefault(x => x.MessageId == inData.MessageId);
                        if (addedAlready != null)
                        {
                            throw new RmsAlreadyExistException(string.Format("MessageId [{0}] はすでに追加済みです。", inData.MessageId));
                        }

                        var dbdata = db.DtDxaQcLog.Add(entity).Entity;
            ...
            {
                throw new RmsException("DT_DXA_QC_LOGテーブルへのInsertに失敗しました。", e);
            }
            finally
            {
