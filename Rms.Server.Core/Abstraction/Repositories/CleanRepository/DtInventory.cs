...
{
    public partial class DtInventoryRepository : ICleanRepository
    {
        /// <summary>
        ...
        public int DeleteExceedsMonthsAllData(DateTime comparisonSourceDatetime)
        {
            ...
            try
            {
                ...
                _dbPolly.Execute(() =>
                {
                    using (DBAccessor.Models.RmsDbContext db = new DBAccessor.Models.RmsDbContext(_appSettings))
                    {
                        ...
                        var targets = db.DtInventory
                        .FromSql(
                            @"
                            Select
                                *
                            From
                                core.DT_INVENTORY tbl1
                            Where
                                tbl1.COLLECT_DATETIME < @CollectDatetime
                                and
                                tbl1.COLLECT_DATETIME <> (SELECT MAX(tbl2.COLLECT_DATETIME) FROM core.DT_INVENTORY tbl2 Where tbl1.DEVICE_SID = tbl2.DEVICE_SID)",
                            CollectDatetime);

                        db.DtInventory.RemoveRange(targets);
            ...
            {
                throw new RmsException("DT_INVENTORYテーブルのDeleteに失敗しました。", e);
            }
            finally
            {
