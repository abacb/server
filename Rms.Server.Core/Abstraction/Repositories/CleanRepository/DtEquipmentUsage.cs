...
{
    public partial class DtEquipmentUsageRepository : ICleanRepository
    {
        /// <summary>
        ...
        public int DeleteExceedsMonthsAllData(DateTime comparisonSourceDatetime)
        {
            ...
            try
            {
                ...
                _dbPolly.Execute(() =>
                {
                    using (DBAccessor.Models.RmsDbContext db = new DBAccessor.Models.RmsDbContext(_appSettings))
                    {
                        ...
                        var targets = db.DtEquipmentUsage
                        .FromSql(
                            @"
                            Select
                                *
                            From
                                core.DT_EQUIPMENT_USAGE tbl1
                            Where
                                tbl1.COLLECT_DATETIME < @CollectDatetime
                                and
                                tbl1.COLLECT_DATETIME <> (SELECT MAX(tbl2.COLLECT_DATETIME) FROM core.DT_EQUIPMENT_USAGE tbl2 Where tbl1.DEVICE_SID = tbl2.DEVICE_SID)",
                            CollectDatetime);

                        db.DtEquipmentUsage.RemoveRange(targets);
            ...
            {
                throw new RmsException("DT_EQUIPMENT_USAGEテーブルのDeleteに失敗しました。", e);
            }
            finally
            {
