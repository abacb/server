...
{
    public partial class DtPlusServiceBillLogRepository : ICleanRepository
    {
        /// <summary>
        ...
        public int DeleteExceedsMonthsAllData(DateTime comparisonSourceDatetime)
        {
            ...
            try
            {
                ...
                _dbPolly.Execute(() =>
                {
                    using (DBAccessor.Models.RmsDbContext db = new DBAccessor.Models.RmsDbContext(_appSettings))
                    {
                        ...
                        var targets = db.DtPlusServiceBillLog
                        .FromSql(
                            @"
                            Select
                                *
                            From
                                core.DT_PLUS_SERVICE_BILL_LOG tbl1
                            Where
                                tbl1.COLLECT_DATETIME < @CollectDatetime
                                and
                                tbl1.COLLECT_DATETIME <> (SELECT MAX(tbl2.COLLECT_DATETIME) FROM core.DT_PLUS_SERVICE_BILL_LOG tbl2 Where tbl1.DEVICE_SID = tbl2.DEVICE_SID)",
                            CollectDatetime);

                        db.DtPlusServiceBillLog.RemoveRange(targets);
            ...
            {
                throw new RmsException("DT_PLUS_SERVICE_BILL_LOGテーブルのDeleteに失敗しました。", e);
            }
            finally
            {
