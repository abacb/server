...
{
    public partial class DtDxaQcLogRepository : ICleanRepository
    {
        /// <summary>
        ...
        public int DeleteExceedsMonthsAllData(DateTime comparisonSourceDatetime)
        {
            ...
            try
            {
                ...
                _dbPolly.Execute(() =>
                {
                    using (DBAccessor.Models.RmsDbContext db = new DBAccessor.Models.RmsDbContext(_appSettings))
                    {
                        ...
                        var targets = db.DtDxaQcLog
                        .FromSql(
                            @"
                            Select
                                *
                            From
                                core.DT_DXA_QC_LOG tbl1
                            Where
                                tbl1.COLLECT_DATETIME < @CollectDatetime
                                and
                                tbl1.COLLECT_DATETIME <> (SELECT MAX(tbl2.COLLECT_DATETIME) FROM core.DT_DXA_QC_LOG tbl2 Where tbl1.DEVICE_SID = tbl2.DEVICE_SID)",
                            CollectDatetime);

                        db.DtDxaQcLog.RemoveRange(targets);
            ...
            {
                throw new RmsException("DT_DXA_QC_LOGテーブルのDeleteに失敗しました。", e);
            }
            finally
            {
